...
function Admin() {
  ...
  const fetchItems = async () => {
    try {
      ...
      setIsFetching(false); // Stop loader when done
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(error);
      const errorMsg = error.response?.data?.message || error.response?.data?.errors?.[0]?.msg || "Failed to add item. Check server console.";
  ...
  const deleteItem = async (id) => {
    // ... [KEEP YOUR EXISTING deleteItem CODE HERE] ...
    try {
  ...
  const handleUpdateStock = async (id, currentStock, itemName) => {
    // ... [KEEP YOUR EXISTING handleUpdateStock CODE HERE] ...
